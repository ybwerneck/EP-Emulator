...
from src.surrogate_models.neural_networks import NModel  # Ensure this matches the location of your NModel class
...
architectures = {
    "small": [(nn.SiLU, 16)],  # 2 layers, 128 neurons each
    "medium": [(nn.ELU, 32)] * 2,               # 4 layers, 256 neurons each
    "large": [(nn.SiLU, 64)] * 4             # 8 layers, 512 neurons each
}
...
for data_set in sets:
    ...
    for name, hidden_layer_sizes in architectures.items():
        ...
        model = NModel(input_size, output_size, hidden_layer_sizes, learning_rate, epochs)
        model.train(x_train, y_train, epochs=-1, patience=1000,tol=1e-5)
