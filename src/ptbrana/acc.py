...
# Chaves dos arquivos (usadas para carregar os CSVs)
probs = ["A", "B", "C", "D"]

# Rótulos em LaTeX para exibição
latex_labels = {
    "A": r"$A_E$",
    "B": r"$B_E$",
    "C": r"$A_M$",
...
groups = [
    ["A", "B"],   # subplot superior -> A_E, B_E
    ["C", "D"]    # subplot inferior -> A_M, B_M
]
...
for ax, group_probs in zip(axes, groups):
    ...
    for i, model in enumerate(model_names):
        model_data = plot_df[
            (plot_df['Modelo'] == model) &
            (plot_df['Problema'].isin(group_probs))
        ].sort_values("Problema")

        ax.bar(
            x + (i - n_models/2) * bar_width + bar_width/2,
            model_data['Media'],
            yerr=model_data['Desvio'],
            width=bar_width * 0.9,
    ...
    ax.set_yscale("log")
    ax.set_ylabel("MARE", fontsize=18)
...
fig.supxlabel("Problema", fontsize=18)
fig.suptitle("Acurácia de validação dos substitutos entre problemas", fontsize=22)
