...
# Sampling for Model C
start = time.time()
dist_B = modelB.getDist(low=.5, high=1.5,disse_l=0.0,disse_h=1)
...
# Write performance CSV for Model C
...
# Sampling for Model C (12 parameters, 0.5–1.5 range)
start = time.time()
dist_C = modelC.getDist(low=0.75, high=1.25)
samplesC = dist_C.sample(sample_size, rule="latin_hypercube")
end = time.time()
sampling_time = end - start
performance_C.append(["Sampling", f"{sampling_time:.4f}"])
print(f"Model C - Sampling: {sampling_time:.4f} seconds")

# Running Model C
start = time.time()
results_C = modelC.run(samplesC.T)
end = time.time()
run_time = end - start
performance_C.append(["Running Model", f"{run_time:.4f}"])
print(f"Model C - Running Model: {run_time:.4f} seconds")
...
qoi_time = end - start
performance_C.append(["Extracting QOIs", f"{qoi_time:.4f}"])
print(f"Model C - Extracting QOIs: {qoi_time:.4f} seconds")
...
save_time = end - start
performance_C.append(["Saving Data", f"{save_time:.4f}"])
print(f"Model C - Saving Data: {save_time:.4f} seconds")
...
with open(csv_path, mode="w", newline="") as csvfile:
    writer = csv.writer(csvfile)
    writer.writerow(["Step", "Time (seconds)"])
    writer.writerows(performance_C)
