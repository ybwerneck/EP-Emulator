...
def plot_qoi_histograms_for_two_models(file_paths):
    ...
    scales = [
        (-50, 100),      # V_peak: allow extreme depolarization/hyperpolarization
        (-150, 0),       # V_rest: very negative resting potentials
        (3000, 7000),    # Ca_peak: large spread around SR/Ca transients
        (0, 600),        # APD90/50: much longer durations (e.g., diseased models)
        (0, 600),        # APD30: same
        (-0.5, 1.5)      # Normalized biomarkers: over- and under-shoot
    ]
    # Number of QoIs to plot (excluding the last one)
    num_qois = len(scales)-1
    ...
    num_rows =3  # Two rows, one for each model
...
def process_directory_for_histograms(root_dir):
    ...
    for model_dir in os.walk(root_dir):
        ...
        model_a_path = os.path.join(model_path, "ModelA", "Y.csv")
        model_b_path = os.path.join(model_path, "ModelB", "Y.csv")
        model_c_path = os.path.join(model_path, "ModelC", "Y.csv")
        print(model_dir)
        # Check if both Model A and Model B have a Y.csv file
        if os.path.exists(model_a_path) and os.path.exists(model_b_path) and    os.path.exists(model_c_path):
            file_paths = [model_a_path, model_b_path,model_c_path]
