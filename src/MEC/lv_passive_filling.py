...
from src.MEC.klotz import fitmodel
...
def LVPassiveFilling(geoparams, hoparams, sampleid=0):
	...
	"""
	print("FULL_____________________",hoparams)
	...
	for i in range(nsteps):
		...
		print("---- DG0 / mesh diagnostics ----")
		print("mesh.num_cells()            =", mesh.num_cells())
		print("DG0 function_space dim      =", f_d.function_space().dim())
		print("local vector size           =", vec.local_size())
		print("local_range (lo, hi)        =", vec.local_range())
		print("--------------------------------")
	...
	np.random.seed(2)
	p_rand = np.random.randint(0, mesh.num_cells(), size = 20)
	...
	print("fitting vol")
	print(vn)
	result_vol = fitmodel(vn, outpress, 2, 1, plot_fit=True, plot_fid=sampleid, plot_label='Volume')

	print("fitting def")
	print(outdef_avg)
	print(outpress)
	result_def = fitmodel(outdef_avg, outpress, 2, 1, plot_fit=True, plot_fid=sampleid, plot_label='Strain')
